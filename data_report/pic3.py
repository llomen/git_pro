...
for i in range(2, get_data.bug_max):
    if get_data.ws.cell(i, 16).value != None:
        data.append(float(get_data.ws.cell(i,16).value))
    else:
        data = data
# last_bug = 0
# for i in range(2, get_data.bug_max):
...
for i in range(2,get_data.bug_max):  #获取缺陷创建日期并存入 bug_creat中
        bug_creat.append(float(get_data.ws.cell(i, 3).value))
...
for j in range(len(bug_creat)):
    if data[-1] <= bug_creat[j] :
        ...
        for i in range(l):
            if data[i-1] <= bug_close[j] < data[i]:
                bug_list_creat[i-1] = bug_list_creat[i-1] + 1

#   处理缺陷关闭轮次
for i in range(2,get_data.bug_max):  #获取缺陷关闭日期并存入 bug_close中
    if get_data.ws.cell(i, 6).value != None:
        bug_close.append(float(get_data.ws.cell(i, 6).value))
...
for j in range(len(bug_close)):
    if data[-1] <= bug_creat[j]:
        ...
        for i in range(l):
            if data[i-1] <= bug_close[j] < data[i]:
                bug_list_close[i-1] = bug_list_close[i-1] + 1
