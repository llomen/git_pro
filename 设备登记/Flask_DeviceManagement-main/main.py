...
def log_handle():
    ...
    # 通过mysql进行存储
    db = pymysql.connect(host="localhost", user="root", password="qq969838847", db="www")
...
def log_success():
    db = pymysql.connect(host="localhost", user="root", password="qq969838847", db="device_manager")
    cursor = db.cursor()
...
def devices_add():
    ...
    brrowser = request.form.get('brrowser')
    bpub_date = request.form.get('bpub_date')
    params_list.append(device)
    params_list.append(brrowser)
    params_list.append(bpub_date)
    # 创建Connection连接
    conn = pymysql.connect(host='localhost', port=3306, db='device_manager', user='root', password='qq969838847',
                           charset='utf8')
    # 获得Cursor对象
    cs1 = conn.cursor()

    cs1.execute("insert into devices(device,brrowser,bpub_date) values(%s,%s,%s)",
                params_list)
    # 得到数据库的数据
    conn.commit()
    # 关闭
...
def devices_delete():
    ...
    # 执行查询的sql语句
    cs1.execute("delete from devices where id = %(id)s", params_dict)
...
def devices_update():
    ...
    cs1.execute(
        "update devices set device = %(device)s,brrowser = %(brrowser)s,bpub_date =%("
        "bpub_date)s where id = %(id)s",
        params_dict
    )
